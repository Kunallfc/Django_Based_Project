...
def search(request):
  ...
  #  q = MyUser.objects.get(address=c)
    ...
    try:
        ...
        return HttpResponse('The address does not exists')

  
def home(request):
    r = request.GET.get('name', '')
    s = request.GET.get('address', '')
    t = request.GET.get('username')
    r = str(r)
    s = str(s)
    t = str(t)
    print (r,s,t)
    if r != '':
        x = User.objects.get(username=t)
        a = MyUser.objects.get(user=x)
        print "case 1"
        a.name = r
        a.save()

    if s != '':
        y = User.objects.get(username=t)
        a = MyUser.objects.get(user=y)
        print "case 2"
        a.address = s
        a.save()

    return render(request, 'home.html',{'users':t})
